			...
			<div class="container">
				...
				<div class="breadcrumb-header justify-content-between">
					...
					<div class="d-flex my-auto">
						<a href="packages_register_view.php"><button class="btn btn-light pd-x-20 mg-t-10" type="submit">View Packages</button></a>
					</div>
				</div>
				<!-- /breadcrumb -->
				<!-- row -->
															...
															while($row=mysqli_fetch_assoc($query))
												      		{
												       			extract($row);
												     	 ?>
												        	<option value="<?php echo $row['fld_travel_comp_id'];?>" <?php if($row['fld_travel_comp_id']==$fetch['fld_travel_comp_id']) {echo "selected";} ?>><?php echo $row['fld_travel_comp_name'];?> </option>
												    	<?php 
												    		} 
												    	?>
													</select>
											...
											<div class="form-group mg-b-0">
												...
												<div class="parsley-select" id="slWrapper">
													<select class="form-control select2" data-parsley-class-handler="#slWrapper" data-parsley-errors-container="#slErrorContainer" data-placeholder="Choose one" name="room" id="room" required="">
														...
														<?php
												  			$query=mysqli_query($connect,"select * from tbl_room where fld_hotel_id='".$fetch['fld_hotel_id']."' order by fld_room_name asc ");
												        
															while($row=mysqli_fetch_assoc($query))
												      		{
												       			extract($row);
												     	 ?>
												        	<option value="<?php echo $row['fld_room_id'];?>" <?php if($row['fld_room_id']==$fetch['fld_room_id']) {echo "selected";} ?>><?php echo $row['fld_room_bed_type'];?> </option>
												    	<?php 
												    		} 
												    	?>
													</select>
		...
		if(isset($_POST['update']))
		{
			extract($_POST);
				 ...
				 $up="update tbl_package set 
				 		fld_travel_comp_id='".$_POST['company']."',
				...
				else
				{
					...
					echo "window.location.href='packages_update.php';";
					echo "</script>";
				}					
		}
	?>