							...
							<div class="card-body">
								...
								<form method="POST" class="form-horizontal form-bordered" onsubmit="return validateForm()" enctype="multipart/form-data">
									<?php 
									  ...
									  if(isset($_GET['id']))
										{
										 $query="select p.*,l.*,h.*,t.*,ft.*,s.*,tc.*,cp.* from tbl_package p,tbl_tour_location l, tbl_hotel h,tbl_transport_mode t,tbl_food_type ft,tbl_spot s,tbl_travel_company tc,tbl_custumize_package cp
			                                     		where
		                                      			...
		                                      			cp.fld_package_delete=0 and cp.fld_custumize_id='".$_GET['id']."' group by cp.fld_custumize_id order by cp.fld_custumize_id desc";
									...
									<div class="row row-sm">
										<div class="col-6">
											<div class="form-group mg-b-0">
												...
												<div class="parsley-select" id="slWrapper">
													<select class="form-control select2" data-parsley-class-handler="#slWrapper" data-parsley-errors-container="#slErrorContainer" data-placeholder="Choose one" name="food_type" id="food_type" required="" readonly="">
														<option label="Choose one"></option>
														<?php
														                      
														        $query1=mysqli_query($connect,"select f.*,t.* from tbl_food f,tbl_food_type t WHERE f.fld_hotel_id='".$fetch['fld_hotel_id']."' and f.fld_food_type=t.fld_food_type order by f.fld_food_type asc");
														       	while($row1=mysqli_fetch_assoc($query1)){
														                        extract($row1);

														    ?>
														    	<option value="<?php echo $row1['fld_food_id'];?>" <?php if($row1['fld_food_id']==$fetch['fld_food_id']) {echo "selected";} ?>><?php echo $row1['fld_food_type_name'];?></option>
														     <?php  }?>
													</select>
													<div id="slErrorContainer"></div>
												</div>
										...
										<div class="col-6">
											<div class="form-group mg-b-0">
												...
												<div class="parsley-select" id="slWrapper">
													<select class="form-control select2" data-parsley-class-handler="#slWrapper" data-parsley-errors-container="#slErrorContainer" data-placeholder="Choose one" name="room" id="room" required="" readonly="">
														<option label="Choose one" value="">
														</option>
														<?php
									...
									<div class="row row-sm">
										<div class="col-6">
											<div class="form-group mg-b-0">
												...
												<div class="parsley-select" id="slWrapper">
													<select class="form-control select2" data-parsley-class-handler="#slWrapper" readonly="" data-parsley-errors-container="#slErrorContainer" data-placeholder="Choose one" name="transport" id="transport" required="">
										...
										<div class="col-6">
											<div class="form-group mg-b-0">
												...
												<textarea class="form-control" id="packages_rate" name="packages_rate" readonly="" type="text" style="max-height: 38px"><?php echo $fetch['fld_package_rate'];?></textarea>
											</div>
										</div>
									</div><br><hr>
									<div class="row row-sm">
	...
	<?php 	
		if(isset($_POST['submit']))
		{
			extract($_POST);
				 ...
				 $up="update tbl_custumize_package set 
			          	...
			          	fld_package_status='Approved'
			            where fld_custumize_id='".$_GET['id']."'";
				...
				{
					echo "<script>";
					echo "alert('Custumize Package Approved');";
